<script lang="ts">
	...
	function getFilteredTags(
		...
		const normalizedBackendNames = new Set(availableNames.map((name) => name.toLowerCase().replace(/[.\s]/g, '')));
		...
		return availableFilterTags.filter((tag) => {
			...
			const isAvailableInBackend =
				normalizedBackendNames.has(normalizedTagName) || normalizedBackendNames.has(tag.code.toLowerCase());
